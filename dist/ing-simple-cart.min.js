...
var defineProperty = _defineProperty;var basket = [
	...
	},
	{
		...
		phoneNumberRequired: false,
		ageRestriction: false,
		title: "My Second Voucher",
		price: 50,
		oldPrice: 65,
		availableStock: 301,
		mediaUrl: "max-gift-card.png",
		quantity: 1,
		extraCostValue: 0
	},
	{
		...
		phoneNumberRequired: false,
		ageRestriction: false,
		title: "My Third Voucher",
		price: 50,
		oldPrice: 65,
		availableStock: 301,
		mediaUrl: "GiftVouchers1.jpg",
		quantity: 1,
		extraCostValue: 0
	},
	{
...
};var data=/*#__PURE__*/Object.freeze({__proto__:null,basket: basket,basketSummary: basketSummary,'default': product});var firstName = "Hr J Ritsma";
var lastName = "whyte";
var addresses = [
	{
		addressIndex: 1,
		street1: "Pietje puk straat",
		street2: "Pietje puk straat",
		houseNumber: "6",
		houseNumberAddition: "",
		postalCode: "1335HK",
		city: "Almere"
...
var personalEmailAddress = "j.ritsma@ing.com";
var phoneNumber = "0031-123456789";
var user = {
	firstName: firstName,
	lastName: lastName,
	addresses: addresses,
...
};var userData=/*#__PURE__*/Object.freeze({__proto__:null,firstName: firstName,lastName: lastName,addresses: addresses,personalEmailAddress: personalEmailAddress,phoneNumber: phoneNumber,'default': user});var STORAGE_KEY = '__cart';
...
LitElement.render = render$1;const appliedClassMixins = new WeakMap();
...
let compatibleShadyCSSVersion$1 = true; // @ts-ignore
...
const transformTemplate$1 = (template, scopedElements, templateCache, tagsCache) => new TemplateResult(transform(template.strings, scopedElements, templateCache, tagsCache), transformArray(template.values, scopedElements, templateCache, tagsCache), template.type, template.processor);
/**
...
const DisabledWithTabIndexMixin = dedupeMixin(DisabledWithTabIndexMixinImplementation);/* eslint-disable class-methods-use-this */
/**
...
function checkChrome(flavor = 'google-chrome') {
  ...
  const vendorName = winNav.vendor; // @ts-ignore
...
const FormRegisteringMixinImplementation = superclass => class extends superclass {
  ...
  connectedCallback() {
    ...
    this.dispatchEvent(new CustomEvent('form-element-register', {
      detail: {
        element: this
      },
      bubbles: true
...
const FormRegisteringMixin = dedupeMixin(FormRegisteringMixinImplementation);/* eslint-disable no-bitwise */
...
function getAriaElementsInRightDomOrder(descriptionElements, {
  reverse
} = {}) {
  /**
   * @param {HTMLElement} a
...
class FormControlMixin extends FormRegisteringMixin(DisabledMixin(SlotMixin(superclass))) {
  static get properties() {
    return {
       ...
       */
      label: String,
      // FIXME: { attribute: false } breaks a bunch of tests, but shouldn't...
  ...
  get _feedbackNode() {
    return (
      /** @type {import('./validate/LionValidationFeedback').LionValidationFeedback | undefined} */
      this.__getDirectSlotChild('feedback')
    );
  }
  ...
  updated(changedProperties) {
    ...
    if (changedProperties.has('_ariaLabelledNodes')) {
      this.__reflectAriaAttr('aria-labelledby', this._ariaLabelledNodes, this.__reorderAriaLabelledNodes);
    }

    if (changedProperties.has('_ariaDescribedNodes')) {
      this.__reflectAriaAttr('aria-describedby', this._ariaDescribedNodes, this.__reorderAriaDescribedNodes);
    }
    ...
    if (changedProperties.has('name')) {
      this.dispatchEvent(new CustomEvent('form-element-name-changed', {
        detail: {
          oldName: changedProperties.get('name'),
          newName: this.name
        },
        bubbles: true
      }));
  ...
  _enhanceLightDomA11y() {
    ...
    if (_labelNode) {
      ...
      this.addToAriaLabelledBy(_labelNode, {
        idPrefix: 'label'
      });
    }

    if (_helpTextNode) {
      this.addToAriaDescribedBy(_helpTextNode, {
        idPrefix: 'help-text'
      });
    }
    ...
    if (_feedbackNode) {
      ...
      this.addToAriaDescribedBy(_feedbackNode, {
        idPrefix: 'feedback'
      });
    }
  ...
  _enhanceLightDomA11yForAdditionalSlots(additionalSlots = ['prefix', 'suffix', 'before', 'after']) {
    additionalSlots.forEach(additionalSlot => {
      ...
      if (element) {
        if (element.hasAttribute('data-label') === true) {
          this.addToAriaLabelledBy(element, {
            idPrefix: additionalSlot
          });
        }

        if (element.hasAttribute('data-description') === true) {
          this.addToAriaDescribedBy(element, {
            idPrefix: additionalSlot
          });
        }
      }
  ...
  __reflectAriaAttr(attrName, nodes, reorder) {
    if (this._inputNode) {
      if (reorder) {
        ...
        const outsideNodes = nodes.filter(n => !this.contains(n)); // eslint-disable-next-line no-param-reassign
  ...
  _inputGroupPrefixTemplate() {
    return !Array.from(this.children).find(child => child.slot === 'prefix') ? nothing : html`
            <div class="input-group__prefix">
              <slot name="prefix"></slot>
            </div>
          `;
  ...
  _inputGroupSuffixTemplate() {
    return !Array.from(this.children).find(child => child.slot === 'suffix') ? nothing : html`
            <div class="input-group__suffix">
              <slot name="suffix"></slot>
            </div>
          `;
  ...
  _isEmpty(modelValue = this.modelValue) {
    let value = modelValue; // @ts-ignore
  ...
  static get styles() {
    const superCtor =
    /** @type {typeof import('@lion/core').LitElement} */
    super.prototype.constructor;
    return [superCtor.styles ? superCtor.styles : [], css`
          /**********************
            {block} .form-field
           ********************/
  ...
  addToAriaLabelledBy(element, customConfig = {}) {
    ...
    if (!this._ariaLabelledNodes.includes(element)) {
      this._ariaLabelledNodes = [...this._ariaLabelledNodes, element]; // This value will be read when we need to reflect to attr
  ...
  addToAriaDescribedBy(element, customConfig = {}) {
    ...
    if (!this._ariaDescribedNodes.includes(element)) {
      this._ariaDescribedNodes = [...this._ariaDescribedNodes, element]; // This value will be read when we need to reflect to attr
  ...
  __repropagateChildrenValues(ev) {
    ...
    const isEndpoint = this._isRepropagationEndpoint || this._repropagationRole === 'choice-group'; // Prevent eternal loops after we sent the event below.
    ...
    const formPath = [...parentFormPath, this]; // C2. Finally, redispatch a fresh model-value-changed event from our host, consumable
  ...
  _repropagationCondition(target) {
    return !(this._repropagationRole === 'choice-group' && // @ts-expect-error multipleChoice is not directly available but only as side effect
    !this.multipleChoice && !target.checked);
  }
  /**
   * @overridable
...
const FocusMixinImplementation = superclass => // @ts-expect-error false positive for incompatible static get properties. Lit-element merges super properties already for you.
...
class FocusMixin extends FormControlMixin(superclass) {
  ...
  __teardownEventsForFocusMixin() {
    this._inputNode.removeEventListener('focus',
    /** @type {EventListenerOrEventListenerObject} */
    this.__redispatchFocus);

    this._inputNode.removeEventListener('blur',
    /** @type {EventListenerOrEventListenerObject} */
    this.__redispatchBlur);

    this._inputNode.removeEventListener('focusin',
    /** @type {EventListenerOrEventListenerObject} */
    this.__redispatchFocusin);

    this._inputNode.removeEventListener('focusout',
    /** @type {EventListenerOrEventListenerObject} */
...
const FocusMixin = dedupeMixin(FocusMixinImplementation);const sym = Symbol.for('lion::SingletonManagerClassStorage');
...
}const singletonManager = new SingletonManagerClass();function createCommonjsModule$1(fn, module) {
  return module = {
    exports: {}
...
var formatMessageInterpret = createCommonjsModule$1(function (module, exports) {
  /*::
  import type {
    AST,
    SubMessages
...
var formatMessageParse = createCommonjsModule$1(function (module, exports) {
  /*::
...
let localize = singletonManager.get('@lion/localize::localize::0.10.x') || new LocalizeManager({
...
const SyncUpdatableMixinImplementation = superclass => class extends superclass {
  constructor() {
    super(); // Namespace for this mixin that guarantees naming clashes will not occur...
  ...
  __syncUpdatableInitialize() {
    ...
    this.constructor;
    ns.initialized = true; // Empty queue...
  ...
  requestUpdateInternal(name, oldValue) {
    ...
    const ctor =
    /** @type {typeof SyncUpdatableMixin & typeof import('../../types/utils/SyncUpdatableMixinTypes').SyncUpdatableHost} */
    this.constructor; // Before connectedCallback: queue

    if (!ns.connected) {
      ns.queue = ns.queue || new Set(); // Makes sure that we only initialize one time, with most up to date value
...
class LionValidationFeedback extends LitElement {
  ...
  render() {
    return html`
      ${this.feedbackData && this.feedbackData.map(({
      message,
      type,
      validator
    }) => html`
...
class Validator {
  ...
  async _getMessage(data) {
    ...
    if (this.config.getMessage) {
      ...
      throw new Error(`You must provide a value for getMessage of type 'function', you provided a value of type: ${typeof this.config.getMessage}`);
    }
...
const ValidateMixinImplementation = superclass => // @ts-expect-error false positive for incompatible static get properties. Lit-element merges super properties already for you.
class extends FormControlMixin(SyncUpdatableMixin(DisabledMixin(SlotMixin(ScopedElementsMixin(superclass))))) {
  static get scopedElements() {
    const scopedElementsCtor =
    /** @type {typeof import('@open-wc/scoped-elements/src/types').ScopedElementsHost} */
    super.constructor;
    return { ...scopedElementsCtor.scopedElements,
  ...
  get slots() {
     ...
     */
    const ctor =
    /** @type {typeof ValidateMixin & ScopedElementsObj} */
    this.constructor;
    return { ...super.slots,
  ...
  updateSync(name, oldValue) {
    ...
    if (['touched', 'dirty', 'prefilled', 'focused', 'submitted', 'hasFeedbackFor', 'filled'].includes(name)) {
    ...
    if (name === 'showsFeedbackFor') {
      ...
      if (this._inputNode) {
        this._inputNode.setAttribute('aria-invalid', `${this._hasFeedbackVisibleFor('error')}`); // this._inputNode.setCustomValidity(this._validationMessage || '');
      ...
      if (diff.length > 0) {
        this.dispatchEvent(new Event(`showsFeedbackForChanged`, {
          bubbles: true
        }));
      }

      diff.forEach(type => {
        this.dispatchEvent(new Event(`showsFeedbackFor${pascalCase(type)}Changed`, {
          bubbles: true
        }));
      });
    }
    ...
    if (name === 'shouldShowFeedbackFor') {
      ...
      if (diff.length > 0) {
        this.dispatchEvent(new Event(`shouldShowFeedbackForChanged`, {
          bubbles: true
        }));
      }
    }
  ...
  async __executeValidators() {
    ...
    const value = this.modelValue instanceof Unparseable ? this.modelValue.viewValue : this.modelValue;
    ...
    const
    /** @type {Validator[]} */
    filteredValidators = this._allValidators.filter(v => !(v instanceof ResultValidator) && !(v instanceof Required));

    const
    /** @type {Validator[]} */
    syncValidators = filteredValidators.filter(v => {
      const vCtor =
      /** @type {typeof Validator} */
      v.constructor;
      return !vCtor.async;
    ...
    const
    /** @type {Validator[]} */
    asyncValidators = filteredValidators.filter(v => {
      const vCtor =
      /** @type {typeof Validator} */
      v.constructor;
      return vCtor.async;
  ...
  async __executeAsyncValidators(asyncValidators, value) {
    if (asyncValidators.length) {
      ...
      this.__asyncValidationResult = booleanResults.map((r, i) => asyncValidators[i]) // Create an array of Validators
  ...
  __finishValidation({
    ...
    const syncAndAsyncOutcome = [...this.__syncValidationResult, ...this.__asyncValidationResult]; // if we have any ResultValidators left, now is the time to run them...

    const resultOutCome = this.__executeResultValidators(syncAndAsyncOutcome);

    this.__validationResult = [...resultOutCome, ...syncAndAsyncOutcome]; // this._storeResultsOnInstance(this.__validationResult);
    ...
    const validationStates = ctor.validationTypes.reduce((acc, type) => ({ ...acc,
      [type]: {}
    }), {});
    ...
    this.dispatchEvent(new Event('validate-performed', {
      bubbles: true
    }));
  ...
  __setupValidators() {
    ...
    this._allValidators.forEach(v => {
      if (!(v instanceof Validator)) {
        ...
        const errorMessage = `Validators array only accepts class instances of Validator. Type "${errorType}" found. This may be caused by having multiple installations of @lion/form-core.`; // eslint-disable-next-line no-console
      ...
      if (ctor.validationTypes.indexOf(v.type) === -1) {
        const vCtor =
        /** @type {typeof Validator} */
        v.constructor; // throws in constructor are not visible to end user so we do both
  ...
  __isEmpty(v) {
    ...
    return this.modelValue === null || typeof this.modelValue === 'undefined' || this.modelValue === '';
  } // ------------------------------------------------------------------------------------------
  // -- Feedback specifics --------------------------------------------------------------------
  // ------------------------------------------------------------------------------------------
  ...
  _hasFeedbackVisibleFor(type) {
    return this.hasFeedbackFor && this.hasFeedbackFor.includes(type) && this.shouldShowFeedbackFor && this.shouldShowFeedbackFor.includes(type);
  }
  /** @param {import('@lion/core').PropertyValues} changedProperties */
  ...
  updated(changedProperties) {
    ...
    if (changedProperties.has('shouldShowFeedbackFor') || changedProperties.has('hasFeedbackFor')) {
      const ctor =
      /** @type {typeof import('../../types/validate/ValidateMixinTypes').ValidateHost} */
      this.constructor; // Necessary typecast because types aren't smart enough to understand that we filter out undefined

      this.showsFeedbackFor =
      /** @type {string[]} */
      ctor.validationTypes.map(type => this._hasFeedbackVisibleFor(type) ? type : undefined).filter(_ => !!_);
  ...
  _updateShouldShowFeedbackFor() {
    ...
    this.shouldShowFeedbackFor =
    /** @type {string[]} */
    ctor.validationTypes.map(type => this._showFeedbackConditionFor(type) ? type : undefined).filter(_ => !!_);
  }
  /**
   * @overridable
  ...
  _prioritizeAndFilterFeedback({
    ...
    /** @type {typeof import('../../types/validate/ValidateMixinTypes').ValidateHost} */
    this.constructor;
    const types = ctor.validationTypes; // Sort all validators based on the type provided.

    const res = validationResult.filter(v => this._showFeedbackConditionFor(v.type)).sort((a, b) => types.indexOf(a.type) - types.indexOf(b.type));
    return res.slice(0, this._visibleMessagesAmount);
  }

};
const ValidateMixin = dedupeMixin(ValidateMixinImplementation);/* eslint-disable class-methods-use-this */
/**
...
const FormatMixinImplementation = superclass => // @ts-expect-error false positive for incompatible static get properties. Lit-element merges super properties already for you.
class FormatMixin extends ValidateMixin(FormControlMixin(superclass)) {
  ...
  __callFormatter() {
    ...
    if (this.__isHandlingUserInput && this.hasFeedbackFor && this.hasFeedbackFor.length && this.hasFeedbackFor.includes('error') && this._inputNode) {
  ...
  connectedCallback() {
    ...
    this.addEventListener('user-input-changed', this._onUserInputChanged); // Connect the value found in <input> to the formatting/parsing/serializing loop as a
  ...
  disconnectedCallback() {
    ...
    if (this._inputNode) {
      ...
      this._inputNode.removeEventListener(this.formatOn,
      /** @type {EventListenerOrEventListenerObject} */
      this._reflectBackFormattedValueDebounced);
    }
  }
...
const InteractionStateMixinImplementation = superclass => // @ts-expect-error false positive for incompatible static get properties. Lit-element merges super properties already for you.
class InteractionStateMixin extends FormControlMixin(superclass) {
  ...
  _onTouchedChanged() {
    this.dispatchEvent(new CustomEvent('touched-changed', {
      bubbles: true,
      composed: true
    }));
  }
...
class LionField extends FormControlMixin(InteractionStateMixin(FocusMixin(FormatMixin(ValidateMixin(SlotMixin(LitElement)))))) {
  static get properties() {
    return {
      autocomplete: {
        type: String,
...
const NativeTextFieldMixinImplementation = superclass => class NativeTextFieldMixin extends superclass {
  /** @type {number} */
  get selectionStart() {
    const native = this._inputNode;
    ...
    if (this.focused) {
      ...
      try {
        ...
        if (!(this._inputNode instanceof HTMLSelectElement)) {
          ...
          this._inputNode.value = newValue; // The cursor automatically jumps to the end after re-setting the value,
...
}customElements.define('lion-input', LionInput);const isKeyboardClickEvent =
/** @type {KeyboardEvent} */
e => e.key === ' ' || e.key === 'Enter';

const isSpaceKeyboardClickEvent =
/** @type {KeyboardEvent} */
e => e.key === ' ';

class LionButton extends DisabledWithTabIndexMixin(SlotMixin(LitElement)) {
  static get properties() {
    return {
      role: {
        type: String,
  ...
  connectedCallback() {
    ...
    this.__setupEvents(); // Old browsers (IE11, Old Edge, Firefox ESR 60) don't have the `.form`
  ...
  async __clickDelegationHandler(ev) {
    ...
    if ((this.type === 'submit' || this.type === 'reset') && ev.target === this && this._form) {
      ...
      this._form.appendChild(this.__submitAndResetHelperButton); // Form submission or reset will happen
...
}customElements.define('lion-button', LionButton);function ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); if (enumerableOnly) symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; }); keys.push.apply(keys, symbols); } return keys; }
...
var add = function add(product, quantity) {
  return isValid$1(product) ? exists(product.productId) ? update(product.productId, 'quantity', get(product.productId).quantity + (quantity || 1)) : save(list().concat(_objectSpread(_objectSpread({}, product), {}, {
    quantity: quantity || 1
  }))) : null;
};
...
var isValid$1 = function isValid(product) {
...
var cartCount = function cartCount() {
  return list().length;
};exports.add=add;exports.cartCount=cartCount;exports.destroy=destroy;exports.exists=exists;exports.get=get;exports.list=list;exports.onChange=onChange;exports.products=products;exports.quantity=quantity;exports.remove=remove;exports.subtotal=subtotal;exports.total=total;exports.update=update;exports.user=user$1;Object.defineProperty(exports,'__esModule',{value:true});return exports;}({}));//# sourceMappingURL=ing-simple-cart.min.js.map
